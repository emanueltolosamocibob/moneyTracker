...
**La tabla de "Alertas de Telegram" (`api/telegram/buy-alerts.ts`) lee directo de `trade_signals`**, ya parseado por regex al ingerir (ver `api/_lib/parseSignal.ts` — el mismo pipeline que alimenta el paper trading), sin costo de LLM. Arma cada fila con `extractDisplayFields` (nombre de compañía, re-derivado de `raw_text`, no una columna propia) más el `% desde la alerta` vía `createPriceLookup`. Hubo una feature anterior (`api/telegram/analyze.ts`, resumen narrativo por LLM con botón "Analizar") que se eliminó por decisión de producto — quedó documentada acá solo porque explica por qué la tabla, aun compartiendo pantalla, nunca dependió de ella: `analyze.ts` mandaba hasta `MAX_MESSAGES=300` mensajes de chat crudo a un LLM, y con ~13 mensajes/día de charla en este canal esos 300 son apenas ~3 semanas — elegir "1 año" en el selector de período no cambiaba nada, siempre se veía "este mes". La tabla de `trade_signals` no tiene ese límite.

**"Estado" (Abierta/Cerrada) y "Fecha de venta"** se derivan recorriendo el historial completo de `trade_signals` por símbolo en orden cronológico (no solo el período pedido — una compra vieja cerrada hace poco tiene que verse cerrada igual): cada compra abre una posición, cada venta cierra la más reciente de ese símbolo. Con doble click sobre una fila se puede además cerrar una alerta a mano (`trade_signals.manual_sell_date`, migración `0018`) para el caso en que el usuario vendió sin que el canal mandara su propia alerta de venta — una alerta de venta real siempre tiene prioridad sobre el cierre manual si después aparece. El PATCH de edición vive en el mismo archivo que el GET (`api/telegram/buy-alerts.ts`), no en uno nuevo — el plan Hobby está al tope de 12 funciones (ver más abajo), mismo patrón que `api/paper/summary.ts` (GET+POST). Para una alerta cerrada, el `% desde la alerta` mide la operación real (entrada → fecha de cierre) en vez de hasta hoy — `createPriceLookup` acepta un `asOfDate` opcional para esto.
...
**Cada alerta de compra abre la misma posición en varias estrategias a la vez, no solo una.** Un análisis previo del histórico de este canal (84 alertas, ene-ago 2026) encontró que el problema no era qué se compraba sino cuándo se vendía: comprar y no tocar ganó +8,32% por operación, mientras que una regla de salida fija (+3% take profit / -3% stop loss) dio -0,96% con costos, porque el rango diario promedio de este universo (3,62%) ya supera ese stop — el 79% de las posiciones "stopeadas" después habían superado el objetivo. Con una sola estrategia corriendo no hay forma de distinguir criterio de ruido desde adentro, así que `api/_lib/paperStrategies.ts` define seis en paralelo sobre la misma señal: `llm` (discrecional, la única que decide el modelo), `buy_hold`, `tp3_sl3`, `channel_levels` (los niveles que la propia alerta declara), `channel_exit` (sale cuando el canal publica su alerta de venta) y `spy_benchmark` (compra SPY en la misma ventana que `llm`, para aislar el alpha real). Ese período no tuvo un mercado bajista, así que "los stops pierden contra mantener" es una propiedad del régimen medido, no una conclusión general — vale seguir esta grilla corriendo en tiempo real para verlo bajo otro régimen.

**La ingesta corre en tiempo real desde un proceso local (`scripts/telegram-paper-listener.mjs`), no desde el sync por Vercel de la feature de arriba.** El cron del plan Hobby es 1 vez por día; una alerta de compra actuada 24hs tarde ya no es la misma decisión que la que el canal proponía. El listener reusa las mismas credenciales de MTProto (`TELEGRAM_API_ID/HASH/SESSION/CHAT_ID`) pero corre en la máquina del usuario, escucha `NewMessage` en tiempo real y manda cada mensaje a `/api/paper/ingest-message` (autenticado con `TELEGRAM_INGEST_SECRET`, un secreto compartido — el listener no tiene JWT de usuario). `api/cron/paper-evaluate.ts` (diario, después del cierre de mercados de EE.UU.) es la red de seguridad: primero corre `ingestUnprocessedSignals` para agarrar cualquier mensaje que haya llegado por el sync-por-Vercel de la otra feature mientras el listener estaba apagado, y después evalúa las posiciones abiertas.

`api/_lib/parseSignal.ts` extrae ticker/take-profit/stop-loss/etc. con regex, no con el LLM: la plantilla de la alerta es fija y se validó a mano contra el histórico completo (100% de las alertas de compra matchean). El LLM se reserva para lo que sí necesita criterio: `api/_lib/evaluatePosition.ts` (estrategia `llm`) corre una búsqueda de noticias del papel y después decide mantener/vender con eso más el precio, las velas y el rango diario promedio del papel como contexto — mismo patrón de dos pasadas y mismo `throttleGeminiCall` compartido que `categorize.ts`. Cada evaluación se guarda en `paper_decisions`, **también cuando la decisión es mantener** — sin eso no habría forma de auditar el criterio, que es justamente lo que este experimento mide.
