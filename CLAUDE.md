...
### Custom DateField component (mobile only)

`src/components/DateField.tsx` replaces every `<input type="date">` in the app (`Transactions.tsx` ×2, `Investments.tsx` ×3, `Budgets.tsx` ×2 custom-range). It's a mobile-only swap: on desktop it just renders the native `<input type="date">`, unchanged, with its native OS calendar. On mobile (`window.matchMedia('(max-width: 720px)')`, checked at mount and kept in sync via a `change` listener — a real runtime check, not a CSS `display` toggle, so no dead markup for the unused variant) it renders a hand-built button-trigger + calendar popup instead, reusing `Select.tsx`'s exact `.select`/`.select-trigger`/`.select-panel` classes so it looks identical to every other dropdown in the app, with a bit of extra CSS (`.date-field-*`) just for the calendar grid itself.

This exists because iOS Safari's native date input was painting its internal day/month/year segments wider than its own border-box in the mobile-stacked form layout, overflowing off-screen and getting clipped by `.app-shell`'s `overflow: hidden`. Multiple rounds of CSS (`font-size`, `flex-shrink`, `min-width: 0`, `overflow: hidden`, `-webkit-text-size-adjust`) did not fix it on a real device — the eventual conclusion was that this is a genuine WebKit rendering bug, not a sizing/flex miscalculation fixable from CSS, so the control got replaced outright instead (same reasoning as `Select.tsx`/`Modal.tsx` replacing other native controls that can't be restyled). Desktop was never broken, hence the mobile-only split rather than replacing it everywhere.
