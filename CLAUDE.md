...
**Stack**: Vite + React + TypeScript (no framework — plain SPA, React Router), Supabase (Postgres + Auth + Row Level Security), Vercel serverless functions for anything needing a secret, Vercel AI Gateway for LLM calls (no provider API key needed in Vercel deployments — OIDC), Linear for issue tracking.
...
Per connection: refresh the Google access token → build a Gmail search query (`api/_lib/bankSenders.ts`, a hardcoded list of known bank/wallet sender addresses — update this list to match the user's real banks) → the date floor is `max(last_scanned_at, now - 15 days)`, so it never reaches further back than 15 days but stays incremental within that window → for each matching message, extract plain text and hand it to `api/_lib/categorize.ts`.

The 15-day cap and the 3s delay between LLM calls in `scanGmailForUser.ts` both exist for the same reason: Vercel AI Gateway's free tier (no paid credits on the account) rate-limits requests per minute, and a scan with many matching emails would otherwise burn through that limit mid-run and fail with `GatewayRateLimitError`. If the AI Gateway account moves to paid credits, both of these could be relaxed — but the categorization model itself (`categorize.ts`) needs to stay off `anthropic/*` models regardless, since Anthropic models on AI Gateway return "Free tier users do not have access to this model" even with a card on file (confirmed by calling the gateway directly); `openai/gpt-5.4-mini` works on free tier and is used for both the main extraction and the merchant web-search fallback.

`categorize.ts` does one `generateObject` call against the user's *actual* category list (not a fixed enum) plus payment method/last-4-digits extraction. If the first pass is low-confidence or falls back to "Otros", it does a second pass: an `openai.tools.webSearch` call (also via AI Gateway) to research the merchant name, then re-categorizes with that context. The LLM may propose a brand-new category name; `scanGmailForUser.ts` only actually inserts it into `categories` when confidence ≥ `NEW_CATEGORY_CONFIDENCE_THRESHOLD` (0.85, stricter than the 0.6 review threshold) and it isn't a generic catch-all — otherwise the transaction falls back to "Otros" with `needs_review = true`.
