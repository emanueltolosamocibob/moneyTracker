...
(Git Bash/zsh need `shopt -s globstar` or an explicit file list for `**` to recurse — if it silently only picks up the top-level files, list the subdirectories by hand: `api/gmail/*.ts api/cron/*.ts api/telegram/*.ts api/investments/*.ts api/uva/*.ts api/_lib/*.ts`.)

**Every file directly under `api/` (excluding `api/_lib/`, which is helpers, not routes) becomes one Vercel serverless function, and the Hobby plan caps a deployment at 12.** Adding a 13th breaks the build outright: *"No more than 12 Serverless Functions can be added to a Deployment on the Hobby plan"* — happened once already (`api/telegram/buy-alerts.ts` was the 13th; fixed at the time by merging `api/paper/evaluate.ts` into `api/paper/summary.ts` via GET/POST on one handler instead of two files — both of those files are gone now, along with the rest of paper trading, see below). Count with `find api -name "*.ts" -not -path "*/_lib/*" | wc -l` before adding a new route file — if already at 12, fold the new route into an existing one by HTTP method instead of adding a file.
...
El frontend nunca lee `telegram_messages` directamente, solo datos ya derivados (decisión de producto: menos datos de terceros a la vista).

**La tabla de "Alertas de Telegram" (`api/telegram/buy-alerts.ts`) lee directo de `trade_signals`**, ya parseado por regex al sincronizar (ver `api/_lib/signalIngest.ts` y `api/_lib/parseSignal.ts`), sin costo de LLM. Arma cada fila con `extractDisplayFields` (nombre de compañía, re-derivado de `raw_text`, no una columna propia) más el `% desde la alerta` vía `createPriceLookup`. Hubo una feature anterior (`api/telegram/analyze.ts`, resumen narrativo por LLM con botón "Analizar") que se eliminó por decisión de producto — quedó documentada acá solo porque explica por qué la tabla, aun compartiendo pantalla, nunca dependió de ella: `analyze.ts` mandaba hasta `MAX_MESSAGES=300` mensajes de chat crudo a un LLM, y con ~13 mensajes/día de charla en este canal esos 300 son apenas ~3 semanas — elegir "1 año" en el selector de período no cambiaba nada, siempre se veía "este mes". La tabla de `trade_signals` no tiene ese límite.

**"Estado" (Abierta/Cerrada), "Fecha de venta" y "Días"** se derivan recorriendo el historial completo de `trade_signals` por símbolo en orden cronológico (no solo el período pedido — una compra vieja cerrada hace poco tiene que verse cerrada igual): cada compra abre una posición, cada venta cierra la más reciente de ese símbolo. "Días" (frontend, `TelegramAlerts.tsx`) es simplemente la diferencia en días entre la fecha de compra y `sellDate`, sin ida y vuelta al servidor. Con doble click sobre una fila se puede además cerrar una alerta a mano (`trade_signals.manual_sell_date`, migración `0018`) para el caso en que el usuario vendió sin que el canal mandara su propia alerta de venta — una alerta de venta real siempre tiene prioridad sobre el cierre manual si después aparece. El PATCH de edición vive en el mismo archivo que el GET (`api/telegram/buy-alerts.ts`), no en uno nuevo — el plan Hobby está al tope de 12 funciones (ver más abajo). Para una alerta cerrada, el `% desde la alerta` mide la operación real (entrada → fecha de cierre) en vez de hasta hoy — `createPriceLookup` acepta un `asOfDate` opcional para esto.

**Filtros de la tabla** (`TelegramAlerts.tsx`): período y estado, por defecto **30 días / Abiertas** (antes 90 días / Todas — cambiado por preferencia directa, sin otra razón detrás). El período tiene tres presets fijos (30/90/365 días) más un modo **"Personalizado"** con dos `DateField` (desde/hasta, mismo patrón que el rango custom de `Budgets.tsx`) — no dispara la consulta hasta tener las dos fechas cargadas. `GET /api/telegram/buy-alerts` acepta `from`/`to` (`YYYY-MM-DD`) con prioridad sobre `days` cuando vienen los dos.

**El símbolo de cada fila es un link a TradingView** (`tradingViewUrl`/`tradingViewCedearUrl` en `TelegramAlerts.tsx`): un click normal abre **dos pestañas** a la vez — `tradingview.com/symbols/TICKER/` (el papel tal cual, TradingView resuelve el exchange solo) y `tradingview.com/symbols/BCBA-TICKER/` (su CEDEAR en BYMA, mismo ticker con prefijo `BCBA-`, confirmado que TradingView respeta ese formato). Clicks modificados (ctrl/cmd/shift, click del medio) no disparan el `onClick` custom y caen al `href` normal — una sola pestaña, el comportamiento nativo del navegador. Si el ticker no tiene CEDEAR, esa segunda pestaña simplemente muestra el 404 propio de TradingView; no hay validación previa contra ninguna lista de símbolos porque eso requeriría un fetch async antes del `window.open` y la mayoría de los navegadores bloquean como popup una ventana abierta fuera de la cadena síncrona del gesto del usuario.

**El `% desde la alerta` también se puede cargar a mano** (mismo modal de edición, campo visible solo cuando la alerta está cerrada) para cuando Yahoo Finance no tiene serie de precios del símbolo y el cálculo automático da `null` — reusa la columna `reported_result_pct` de `trade_signals` en vez de sumar una nueva: en una fila `kind='sell'` es el % que el canal reportó en su propio mensaje, y en `kind='buy'` (todo lo que edita esta ruta) el parser nunca la pobló, así que estaba libre. Un valor manual tiene prioridad sobre el calculado si ambos existen.

**La tabla también permite agregar y borrar alertas de compra a mano** (botones "+ Agregar alerta" y "Eliminar alerta" en `TelegramAlerts.tsx`, POST/DELETE en el mismo `api/telegram/buy-alerts.ts`). Una alerta cargada a mano no tiene mensaje real de Telegram detrás: `message_id` se genera negativo (los ids reales de Telegram son siempre positivos, así que nunca pisa uno) y `raw_text` se arma en el mismo formato que ya sabe leer `extractDisplayFields`, para no duplicar esa lógica. Se marca con `trade_signals.is_manual` (migración `0019`) y la tabla le pone un tag "Manual" al lado del ticker. El borrado pide confirmación explícita en un modal aparte (a diferencia del resto de los borrados manuales de la app, que no la piden) en vez de ser un botón directo — heredado de cuando también cascadeaba a posiciones de paper trading; se mantuvo como red de seguridad genérica aunque esa cascada ya no exista (ver más abajo).

**`api/_lib/signalIngest.ts`** convierte mensajes de `telegram_messages` en filas de `trade_signals`: `processMessage` parsea uno (`parseSignal.ts`, regex puro, sin LLM) y lo inserta; `ingestUnprocessedSignals` barre todos los mensajes de un chat que todavía no tienen señal. Se llama desde `api/telegram/sync.ts` (botón "Sincronizar") y `api/cron/sync-telegram.ts` (cron diario), justo después de `syncTelegramForUser` — antes este paso vivía en el pipeline de paper trading (ver abajo); al eliminarse ese pipeline, la ingesta de señales se movió acá para que la tabla de alertas siga recibiendo señales nuevas sin depender de nada más.

### Paper trading eliminado (portfolio simulado)

Hubo una feature sobre el mismo canal que abría y cerraba posiciones simuladas de verdad (sin dinero real) en paralelo por estrategia (`llm` discrecional, `buy_hold`, `tp3_sl3`, `channel_levels`, `channel_exit`, `spy_benchmark`), con un listener en tiempo real corriendo localmente bajo pm2 (`scripts/telegram-paper-listener.mjs` + `ecosystem.config.cjs`) y un cron diario de evaluación (`api/cron/paper-evaluate.ts`). Se eliminó por decisión de producto: la estrategia discrecional (`api/_lib/evaluatePosition.ts`) terminaba decidiendo "vender" de forma sistemática porque no siempre podía traer noticias del papel, y el experimento de comparar estrategias no estaba dando una señal útil. Se borraron `api/_lib/paperTrading.ts`, `paperStrategies.ts`, `evaluatePosition.ts`, `api/paper/*.ts`, `api/cron/paper-evaluate.ts`, `src/components/PaperTrading.tsx`, el listener y `ecosystem.config.cjs`, más las tablas `paper_positions`/`paper_decisions` (migración `0020_drop_paper_trading.sql`; `trade_signals` no se tocó, sigue siendo la fuente de "Alertas de Telegram").

En su lugar queda **`src/components/SpyBenchmark.tsx`**, un solo número de referencia (cuánto lleva el SPY en el año calendario) contra `api/investments/spy-benchmark.ts`, que calcula el % con `getDailyBars` (`priceHistory.ts`) filtrando las velas desde el 1 de enero — sin tabla ni motor de simulación propios.
